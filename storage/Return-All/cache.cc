 ...
 */
 ...
 uint64_t spvaddr = 93826501857542;
 uint64_t spaddr = 0;
 
 std::vector<bool> is_exclusive(NUM_CPUS, 1);
 std::vector<int> is_ReExclusive(NUM_CPUS, 0);
 
 std::vector<bool> is_l2c_exclusive(NUM_CPUS, 0);
 ...
 void trackPkt(PACKET &pkt, std::string name, std::string caller) {
   if (pkt.v_address == spvaddr || pkt.v_address == 93826501857541) {
     std::cout << "Packet found at function: " << caller << " in cache " << name << std::endl;
     std::cout << "Printing sq_it\n";
     for (auto x : pkt.sq_index_depend_on_me) {
       std::cout << (uint64_t)(&(*x)) << " ";
     }
     std::cout << "\n";
   }
 ...
 void trackvaddr(uint64_t vaddr, std::string name, std::string caller)
 {
   // std::cout << "V_Addr = " << vaddr << "\n";
   // if (vaddr == spvaddr || vaddr == 93826501857541) {
   //   std::cout << "Found vaddr @ function " << caller << " in cache " << name << std::endl;
   // }
 }
 
 void trackaddr(uint64_t addr, std::string name, std::string caller)
 {
   // std::cout << "V_Addr = " << vaddr << "\n";
   //  if (check_string(name,"L1D")) {
   //   std::cout << "L1D: addr: " << addr << " caller: " << caller << std::endl;
   //  }
   if (addr >> LOG2_BLOCK_SIZE == spaddr >> LOG2_BLOCK_SIZE) {
     std::cout << "Found addr @ function " << caller << " in cache " << name << std::endl;
   }
 }
 ...
 void CACHE::handle_fill()
 {
   while (writes_available_this_cycle > 0) {
     ...
     trackvaddr(fill_mshr->v_address, NAME, "handle_fill");
     trackPkt(*fill_mshr,NAME,"handle_fill");
     trackaddr(fill_mshr->address, NAME, "handle_fill");
     // if (check_string(NAME,"L2C") && way != NUM_WAY && !fill_mshr->isReturned) {
     //   for (auto ret : fill_mshr->to_return) {
     //     ret->return_data(&(*fill_mshr));
     //   }
     //   fill_mshr->isReturned = true;
     // }
     if (is_l2c_exclusive[fill_mshr->cpu] && check_string(NAME, "L2C")) {
     ...
     if (is_exclusive[fill_mshr->cpu] && check_string(NAME, "LLC")) {
       // for (auto ret : fill_mshr->to_return) {
       //   ret->return_data(&(*fill_mshr));
       // }
     ...
     if (check_string(NAME, "L2C") && is_exclusive[fill_mshr->cpu]) {
       CACHE* LLC;
       ...
       //@Shriram, once again, no invalidation for loop hierarchy
       if (LLC_way < LLC->NUM_WAY) {
         LLC->block[LLC_set * (LLC->NUM_WAY) + LLC_way].valid = 0;
         LLC->block[LLC_set * (LLC->NUM_WAY) + LLC_way].l2Hit = 0;
       }
     }
     ...
     if (way != NUM_WAY) {
       ...
       if (check_string(NAME, "L1D") && fill_mshr->type == WRITEBACK) {
         for (auto ret : fill_mshr->to_return) {
           ret->return_data(&(*fill_mshr));
         }
       }
 ...
 void CACHE::handle_writeback()
 {
   while (writes_available_this_cycle > 0) {
     ...
     trackvaddr(handle_pkt.v_address, NAME, "handle_writeback");
     trackPkt(handle_pkt,NAME,"handle_writeback");
     ...
     } else // MISS
     {
       ...
       if (!success)
         return;
       if (check_string(NAME, "L2C") && is_exclusive[handle_pkt.cpu]) {
         CACHE* LLC;
 ...
 void CACHE::handle_read()
 {
   while (reads_available_this_cycle > 0) {
     ...
     trackvaddr(handle_pkt.v_address, NAME, "handle_read");
     trackPkt(handle_pkt,NAME,"handle_read");
     trackaddr(handle_pkt.address, NAME, "handle_read");
     ...
     if (way < NUM_WAY) // HIT
     {
       ...
       if (check_string(NAME, "L2C") && (is_ReExclusive[handle_pkt.cpu] != 0)) {
         block[set * NUM_WAY + way].l2Hit++;
       }
       if (check_string(NAME, "L2C") && is_l2c_exclusive[handle_pkt.cpu]) {
         block[set * NUM_WAY + way].valid = 0;
         block[set * NUM_WAY + way].dirty = 0;
         block[set * NUM_WAY + way].l2Hit = 0;
       }
       if (check_string(NAME, "LLC") && is_exclusive[handle_pkt.cpu]) {
         block[set * NUM_WAY + way].valid = 0;
         block[set * NUM_WAY + way].dirty = 0;
         block[set * NUM_WAY + way].l2Hit = 0;
       }
 ...
 {
   trackvaddr(handle_pkt.v_address, NAME, "readlike_hit");
   trackPkt(handle_pkt,NAME,"readlike_hit");
   ...
   if (is_ReExclusive[handle_pkt.cpu] && check_string(NAME, "LLC")) {
     handle_pkt.l2Hit = hit_block.l2Hit; // by being present in llc already, l2hit of handle pkt >= threshold
   }
 ...
 {
   trackvaddr(handle_pkt.v_address, NAME, "readlike_miss");
   trackPkt(handle_pkt,NAME,"readlike_miss");
   trackaddr(handle_pkt.address, NAME, "readlike_miss");
   DP(if (warmup_complete[handle_pkt.cpu]) {
 ...
 /*
     ...
     Bug Description:
     When we merge requests, the requests may be having different load queue and store queue dependencies
     If we simply merge the requests in our mshr, since the response isn't delivered using our mshr it has no effect
     The result is that even though the response delivered to cpu is for requests a,b,c the cpu thinks it only covers request a
 
     Possible Solutions:
       1] We need to track down the packet in the hierarchy and make modifications to it
           - Impossible to practically do
           - Too many edge cases
           - What if response delivered to CPU already but not DTLB/L1I/L1D
       2] From CPU perspective, I suppose it could be less dumb but I would rather not modify CPU
           - Good God I do not want to modify O3_CPU
           - Just a terrible idea in general, might require changing the entire champsim framework
       3] Make it so that only the DTLB, L1D and L1I are authorized to respond to CPU and the rest of the cache hierarchy shall respond to them
           - What if merging in the rest of the hierarchy?
           - Is the above case possible?
           - Experiment time
 
 */
     trackvaddr(handle_pkt.v_address, NAME, "readlike_miss: mshr entry already found");
     // update fill location
     mshr_entry->fill_level = std::min(mshr_entry->fill_level, handle_pkt.fill_level);
 
     if (handle_pkt.v_address == spvaddr) {
       std::cout << "Printing to_return of mshr\n";
       for (auto x : mshr_entry->to_return) {
         std::cout << (uint64_t)x << " ";
       }
       std::cout << "\nPrinting instr_depend_on_me of mshr\n";
       for (auto x : mshr_entry->sq_index_depend_on_me) {
         std::cout << (uint64_t)(&(*x)) << " ";
       }
       std::cout << "\nPrinting to_return of handle_pkt\n";
       for (auto x : handle_pkt.to_return) {
         std::cout << (uint64_t)x << " ";
       }
       std::cout << "\nPrinting instr_depend_on_me of handle_pkt\n";
       for (auto x : handle_pkt.sq_index_depend_on_me) {
         std::cout << (uint64_t)(&(*x)) << " ";
       }
     }
     ...
     trackvaddr(handle_pkt.v_address, NAME, "readlike_miss: mshr entry not found");
     if (mshr_full) {
       trackvaddr(handle_pkt.v_address, NAME, "readlike_miss fail");
     ...
     if (handle_pkt.fill_level <= fill_level) {
       ...
       if (!(check_string(NAME, "L1D") && (handle_pkt.type == WRITEBACK || handle_pkt.type == RFO))) {
         handle_pkt.to_return.push_back(this); // @ Shriram, keep track of who all to deliver response to
       } else {
 ...
 bool CACHE::filllike_miss(std::size_t set, std::size_t way, PACKET& handle_pkt)
 {
   ...
   trackvaddr(handle_pkt.v_address, NAME, "filllike_miss");
   trackPkt(handle_pkt,NAME,"filllike_miss");
   trackaddr(handle_pkt.address, NAME, "filllike_miss");
 
   if (!bypass) {
     if (evicting_dirty || (is_exclusive[handle_pkt.cpu] && check_string(NAME, "L2C") && fill_block.valid)
         || (is_l2c_exclusive[handle_pkt.cpu] && check_string(NAME, "L1D") && fill_block.valid)) {
       ...
       if ((is_exclusive[handle_pkt.cpu] && check_string(NAME, "L2C") && fill_block.valid)
           || ((is_l2c_exclusive[handle_pkt.cpu] && check_string(NAME, "L1D") && fill_block.valid))) {
         writeback_packet.dirty = fill_block.dirty;
       }
       if (is_ReExclusive[handle_pkt.cpu] != 0 && check_string(NAME, "L2C")) {
         fill_block.l2Hit = 0;
         if (writeback_packet.l2Hit >= is_ReExclusive[handle_pkt.cpu]) {
           auto result = lower_level->add_wq(&writeback_packet);
           block1++;
           if (result == -2)
             return false;
         } else if (writeback_packet.dirty) {
           auto result = DRAM.add_wq(&writeback_packet);
           block0++;
           if (result == -2) {
             return false;
           }
   ...
   } else if (check_string(NAME, "L2C")) {
     std::cerr << "What\n";
     exit(1);
   }
 
   if (is_ReExclusive[handle_pkt.cpu]) {
     fill_block.l2Hit = handle_pkt.l2Hit;
   }
 ...
 uint32_t CACHE::get_way(uint64_t address, uint32_t set)
 {
   ...
   uint32_t dist = std::distance(begin, std::find_if(begin, end, eq_addr<BLOCK>(address, OFFSET_BITS)));
   if (dist < NUM_WAY && block[set * NUM_WAY + dist].valid) {
     return dist;
   } else {
     return NUM_WAY;
   }
 ...
 void CACHE::return_data(PACKET* packet)
 {
   ...
   if (is_ReExclusive[packet->cpu] && check_string(NAME, "L2C")) {
     mshr_entry->l2Hit = packet->l2Hit;
   }
