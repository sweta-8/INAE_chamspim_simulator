 ...
 */
 ...
 int O3_CPU::do_translate_store(std::vector<LSQ_ENTRY>::iterator sq_it)
 {
   ...
   DP(if (warmup_complete[cpu]) {
     std::cout << "virtual_address = " << sq_it->virtual_address << "\n";
     std::cout << "[RTS0] " << __func__ << " instr_id: " << sq_it->instr_id << " rob_index: " << " is popped from to RTS0" << std::endl;
   })
   ...
   sq_it->translated = INFLIGHT;
     ...
     if (sq_it->instr_id == 12) {
       std::cout << " Added to DTLB Read Queue with rq_index = " << rq_index << " vaddr = " << sq_it->virtual_address << "\n";
     }
 ...
 void O3_CPU::handle_memory_return()
 {
   ...
   while (to_read > 0 && !DTLB_bus.PROCESSED.empty()) { // DTLB
     ...
     std::cout << "Translation response for: " << dtlb_entry.address << "\n";
     ...
     for (auto sq_merged : dtlb_entry.sq_index_depend_on_me) {
       ...
       std::cout << "Translation response store queue for: " << dtlb_entry.address << ", sq_merged = " << (uint64_t)(&(*sq_merged)) << " instr_id =  " << dtlb_entry.instr_id << "\n";
