...
extern std::map<std::string,uint64_t>DRAM_stats;
...
void MEMORY_CONTROLLER::operate()
{
  for (auto& channel : channels) {
    ...
    if (channel.active_request != std::end(channel.bank_request) && channel.active_request->event_cycle <= current_cycle) {
      channel.total_delay += (double( current_cycle*1.25 -  channel.active_request->pkt->add_dram_cycle*1.25));
      // std::cout << channel.total_delay << ":" << 1.25*(current_cycle)<< ":"<< channel.active_request->pkt->add_dram_cycle*1.25<<":"<< current_cycle*1.25  - channel.active_request->pkt->add_dram_cycle*1.25<<std::endl;
...
int MEMORY_CONTROLLER::add_rq(PACKET* packet)
{
  ...
  packet->add_dram_cycle =current_cycle;

  *rq_it = *packet;
  rq_it->event_cycle = current_cycle;

  DRAM_stats["Added_to_RQ"]++;
