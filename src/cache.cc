...
//########################### DEBUG TRACKERS BEGIN ################################################

// This is for debugging purposes, turn on the flag to track a singular address's pass through the hierarchy
// #define DEBUG_TRACK
...
void trackPkt(PACKET& pkt, std::string name, std::string caller, bool trackByVirtual, bool loadQueue, bool storeQueue, bool toReturn=true)
{
  if ((pkt.v_address >> LOG2_BLOCK_SIZE == spvaddr >> LOG2_BLOCK_SIZE) && trackByVirtual) {
    ...
    if (loadQueue) {
      std::cout << "Printing lq_it\n";
      for (auto x : pkt.lq_index_depend_on_me) {
        std::cout << (uint64_t)(&(*x)) << " ";
      }
      std::cout << "\n";
    }
    if (toReturn) {
      std::cout << "Printing to_return\n";
      for(auto x:caches){
        std::cout << x->NAME << ":"<< &(*x) << " ";
      }
      std::cout << std::endl;
  ...
  else if ((pkt.address >> LOG2_BLOCK_SIZE) == (spaddr >> LOG2_BLOCK_SIZE) && !trackByVirtual) {
    std::cout << "Packet found at function: " << caller << " in cache " << name << std::endl;
    if (storeQueue) {
      std::cout << "Printing sq_it\n";
      for (auto x : pkt.sq_index_depend_on_me) {
...
void CACHE::handle_writeback()
{
  while (writes_available_this_cycle > 0) {
    ...
    if (way < NUM_WAY) // HIT
    {
      ...
      #ifdef DEBUG_TRACK
      trackaddr(handle_pkt.address, NAME, "writeback hit");
      #endif
    ...
    /*
        ...
        Possible Solutions:
          ...
          3] Make it so that only the DTLB, L1D and L1I are authorized to respond to CPU and the rest of the cache hierarchy shall respond to them
              ...
              - Is the above case possible? <- Nope
              - Experiment time
    ...
    */
    ...
    if (mshr_entry->type == PREFETCH && handle_pkt.type != PREFETCH) {
      ...
      *mshr_entry = handle_pkt;
      mshr_entry->type = PREFETCH;
      mshr_entry->returnToL1 = true;
    ...
    if (handle_pkt.fill_level <= fill_level) {
      ...
      if(fill_level == FILL_L1) it->returnToL1 = true;
    }

    if (handle_pkt.fill_level <= fill_level) {

      if (!(check_string(NAME, "L1D") || check_string(NAME, "ITLB") || check_string(NAME, "DTLB") || check_string(NAME, "L1I"))) {
      ...
      #ifdef DEBUG_TRACK
      trackaddr(handle_pkt.address,NAME,"readlike_miss:600 clearing the to-return");
      #endif
    }
