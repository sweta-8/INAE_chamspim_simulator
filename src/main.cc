...
// Additional stats
std::map<std::string,uint64_t> DRAM_stats;
std::vector<std::map<std::string,uint64_t>> MSHR_FULLS(NUM_CPUS);
...
void print_roi_stats(uint32_t cpu, CACHE* cache)
{
  ...
  for (uint32_t i = 0; i < NUM_TYPES; i++) {
    ...
    if (((cache->NAME == "LLC") || (cache->NAME == "cpu0_L2C")) && i == WRITEBACK) {
      continue;
    }
  ...
  if (TOTAL_ACCESS > 0) {
    ...
    cout << cache->NAME;
    cout << " MSHR FULL EVENTS: " << cache->mshr_full_events << " MSHR MERGE EVENTS: " << cache->mshr_merge_events << endl;
...
void print_dram_stats()
{
  ...
  for (uint32_t i = 0; i < DRAM_CHANNELS; i++) {
    ...
    total_delay_cycle += channel.total_delay;
  }
  ...
  std::cout << "Summary: DRAM events" << std::endl;
  for(auto x:DRAM_stats){
    std::cout << x.first << " : " << x.second << std::endl;
  }
  std::cout << " DBUS congested total cycle: " << total_congested_cycle << std::endl;
  std::cout << " DBUS delay total cycle: " << total_delay_cycle << std::endl; 
...
int main(int argc, char** argv)
{
  ...
  for (unsigned long cpu = 0; cpu < NUM_CPUS; cpu++){
    std::cout << "CPU" << cpu <<  "MSHR FULL: " << std::endl;
    for(auto x:MSHR_FULLS[cpu]){
      std::cout << x.first << " : " << x.second << std::endl;
    }
  }
