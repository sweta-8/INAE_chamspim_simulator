...
LDFLAGS := -static -Wl,-rpath=/usr/lib/x86_64-linux-gnu/libstdc++.so.6
LDLIBS := -L/usr/lib/x86_64-linux-gnu/libstdc++.so.6 -lstdc++

.phony: all clean

all: bin/probabilistic-lru-0.5
...
clean: 
	...
	$(RM) inc/ooo_cpu_modules.inc
	 ...
	 find replacement/probabilistic-lru-0.5 -name \*.o -delete
	 find replacement/probabilistic-lru-0.5 -name \*.d -delete
...
bin/probabilistic-lru-0.5: $(patsubst %.cc,%.o,$(wildcard src/*.cc)) obj/repl_rreplacementDprobabilisticHlruH0_5.a obj/pref_pprefetcherDno.a obj/repl_rreplacementDlru.a obj/pref_pprefetcherDno_instr.a obj/bpred_bbranchDbimodal.a obj/btb_bbtbDbasic_btb.a
	$(CXX) $(LDFLAGS) -o $@ $^ $(LDLIBS)

replacement/probabilistic-lru-0.5/%.o: CFLAGS += -Ireplacement/probabilistic-lru-0.5
replacement/probabilistic-lru-0.5/%.o: CXXFLAGS += -Ireplacement/probabilistic-lru-0.5
replacement/probabilistic-lru-0.5/%.o: CXXFLAGS +=  -Dinitialize_replacement=repl_rreplacementDprobabilisticHlruH0_5_initialize -Dfind_victim=repl_rreplacementDprobabilisticHlruH0_5_victim -Dupdate_replacement_state=repl_rreplacementDprobabilisticHlruH0_5_update -Dreplacement_final_stats=repl_rreplacementDprobabilisticHlruH0_5_final_stats
obj/repl_rreplacementDprobabilisticHlruH0_5.a: $(patsubst %.cc,%.o,$(wildcard replacement/probabilistic-lru-0.5/*.cc)) $(patsubst %.c,%.o,$(wildcard replacement/probabilistic-lru-0.5/*.c))
	@mkdir -p $(dir $@)
	ar -rcs $@ $^
...
-include $(wildcard replacement/probabilistic-lru-0.5/*.d)
